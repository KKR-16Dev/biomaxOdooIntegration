...
class CustomDataBiometricImport(models.Model):
    ...
    zoho_employee_id = fields.Char(string="Employee ID")


class FetchBiomaxDataforOdooDATS(models.Model):
    ...
    def attempt_db_connection(self):
        ...
        sql_query = "SELECT * FROM BiometricDB_Test.dbo.parallel_clone WHERE employeeID IN (83, 21, 65, 86) AND DeviceName IN ('DATS In', 'DATS Out') ORDER BY eventTime;"
        ...
        for record in biomax_records_sorted:  # Use sorted records
            ...
            if is_checkin_str == '1':
                ...
                try:
                    ...
                    _logger.info("✓ Created CHECK-IN for Employee ID %s (Odoo ID: %s) at %s UTC", 
                ...
                try:
                    ...
                    if latest_attendance:
                        ...
                        _logger.info("✓ Updated CHECK-OUT for Employee ID %s (Odoo ID: %s) - Check-in: %s → Check-out: %s UTC", 
                                employee_id_str, odoo_emp_id, latest_attendance.check_in, utc_time_str)
                        checkout_count += 1
                        processed_count += 1
                    else:
