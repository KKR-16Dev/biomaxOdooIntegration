...
class FetchBiomaxDataforOdoo(models.Model):
    ...
    # -------------------------------------------------------------------------
    # TES Night Shift Day Field
    # Logic:
    #   - Check-in between 12:00 AM – 3:59 AM  → Previous day's date
    #   - Check-in between 4:00 PM – 11:59 PM  → Same day's date
    #   - Check-in between 4:00 AM – 3:59 PM   → False (not a night shift)
    #   - No check_in                           → False
    # -------------------------------------------------------------------------
    ...
    # Logic:
    #   - Check-in between 12:00 AM – 3:59 AM  → Previous day's date
    #   - Check-in between 4:00 AM – 11:59 PM  → Same day's date
    #   - No check_in                           → False
    # -------------------------------------------------------------------------
    it_shift_day = fields.Date(
        string='IT Shift Day',
        compute='_compute_it_shift_day',
        store=True
    )
    ...
    @api.depends('check_in')
    def _compute_it_shift_day(self):
        for rec in self:
            if rec.check_in:
                ...
                if hour < 4:
                    # 12:00 AM – 3:59 AM → Previous day's date
                    rec.it_shift_day = (check_in_local - timedelta(days=1)).date()
                else:
                    # 4:00 AM – 11:59 PM → Same day's date
                    rec.it_shift_day = check_in_local.date()
            else:
                rec.it_shift_day = False
    ...
    def biomaxOdooIntegrationMains(self):
        ...
        UpReturnValueA = self.add_odoo_employee_ids(ReturnValueA)
    ...
    def attempt_db_connection(self):
        ...
        # sql_query = "SELECT * FROM dbo.parallel_clone2 WHERE employeeID IN (26) AND DeviceName IN ('DATS In', 'DATS Out', 'TES Workstation', 'Kavi Workstation', 'Kavi In', 'Facility In', 'Facility Out', 'TES Server Room','Korkai In Door','Korkai Out Door','Vanji Out Door','Vanji In Door','Thanjai In Door','Thanjai Out Door','Extended Team In','Extended Team Out' ) AND eventTime >= '2025-12-01 00:00:00' AND eventTime <  '2026-02-01 00:00:00' ORDER BY eventTime;"
        
        # DATS QUERY
        # sql_query = "SELECT * FROM dbo.parallel_clone2 WHERE employeeID IN (51) AND DeviceName IN ( 'DATS In', 'DATS Out') AND eventTime >= '2025-12-01 00:00:00' AND eventTime <  '2026-02-01 00:00:00' ORDER BY eventTime;"
        
        # TES QUERY
        # sql_query = "SELECT * FROM dbo.parallel_clone2 WHERE employeeID IN (1035) AND DeviceName IN ('Kavi Workstation', 'Kavi In', 'Facility In', 'Facility Out', 'TES Server Room', 'Korkai In Door','Korkai Out Door', 'Vanji Out Door','Vanji In Door', 'Thanjai In Door','Thanjai Out Door', 'Extended Team In','Extended Team Out' ) AND eventTime >= '2025-12-01 00:00:00' AND eventTime <  '2026-02-01 00:00:00' ORDER BY eventTime;"
        sql_query = "SELECT * FROM dbo.parallel_clone2 WHERE employeeID IN (1045) AND DeviceName IN ('Kavi Workstation', 'Kavi In', 'Facility In', 'Facility Out', 'TES Server Room', 'Korkai In Door','Korkai Out Door', 'Vanji Out Door','Vanji In Door', 'Thanjai In Door','Thanjai Out Door', 'Extended Team In','Extended Team Out' ) AND eventTime >= '2026-01-20 17:00:00' AND eventTime <  '2026-01-23 05:00:00' ORDER BY eventTime;"

        # IT-DEPT QUERY
        # sql_query = "SELECT * FROM dbo.parallel_clone2 WHERE employeeID IN (51) AND DeviceName IN ( 'DATS In', 'DATS Out', 'Kavi Workstation', 'Kavi In', 'Facility In', 'Facility Out', 'Korkai In Door','Korkai Out Door', 'Vanji Out Door','Vanji In Door', 'Thanjai In Door','Thanjai Out Door', 'Extended Team In','Extended Team Out' ) AND eventTime >= '2025-12-01 00:00:00' AND eventTime <  '2026-02-01 00:00:00' ORDER BY eventTime;"
    ...
    def getandprocess_biomax_data(self, success_result):
        ...
        for record in biomax_records_sorted:
